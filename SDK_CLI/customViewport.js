...
const buildName = process.env.AUTOMATION_BUILD_NAME || "SmartUI Exec Node CustomViewport";
...
let capabilities = {
    ...
    "LT:Options": {
		...
		"project": "Untitled",
		"w3c": true,
        name: "SmartUI Node CustomViewport Test", // name of the test
...
(async function example() {
    ...
    try {
        ...
        await smartuiSnapshot(driver, 'lambdatestHomePage', {
            "web": {
                "customViewports": [
                    { "browser": "chrome", "viewport": [1920] },
                    { "browser": "firefox", "viewport": [1280] },
                    { "browser": "safari", "viewport": [1024] },
                    { "browser": "edge", "viewport": [1200] }
                ]
            }
        });
    } catch (error) {
